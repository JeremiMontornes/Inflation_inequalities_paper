...
\begin{itemize}
  ...
  \item The distributional effect came mainly from measures on housing energy: gas and electricity measures reduced the inflation gap by \textbf{1.4 points}, while fuel measures increased it by \textbf{0.2 point}.
\end{itemize}
\end{frame}
...
\begin{itemize}
  ...
  \item RAS balancing distributes official HICP weights across groups; chained Laspeyres indices preserve cross-country comparability and consistency with headline inflation.
\end{itemize}
\[
P_{g,y,m}
=
\sum_{j \in \mathcal{J}_y}
\omega_{j,g,y}\frac{p_{j,y,m}}{p_{j,y,0}}.
\]
\[
\underbrace{\sum_g M_{j,g,y}=\bar{\omega}^{\mathrm{HICP}}_{j,y}}
_{\text{product margin}},
\qquad
\underbrace{\sum_{j \in \mathcal{J}_y}M_{j,g,y}=s_{g,y}}
...
\begin{itemize}
  \item We treat each household as an elementary group and combine its 2020 expenditure shares with monthly HICP price changes.
\end{itemize}
\[
P_{i,y,m}
=
...
\centering
\includegraphics[width=0.96\textwidth,height=0.72\textheight,keepaspectratio]{fig/fig_EA20_household_inflation_distribution_2021_2023.png}
...
% 9
\begin{frame}{Inflation Gaps Are Episodic Rather Than Persistent}
\centering
...
\begin{tikzpicture}
  ...
  \node[anchor=north west,font=\scriptsize\bfseries,text=WarmOrange,
        ...
        at ($(policyplot.north west)!0.295!(policyplot.north east)+(0,-0.18)$) {Policy period: June 2021--June 2023};
\end{tikzpicture}
\vspace{-0.2em}
...
% 12
\begin{frame}{Unconventional Fiscal Policies Narrowed the Inflation Gap by 1.1 pp}
\centering
\begin{tikzpicture}
  \node[inner sep=0] (gapplot) {\includegraphics[width=0.88\textwidth,height=0.64\textheight,keepaspectratio]{fig/fig_EA20_2021_2026_counterfactual_income_inflation_Q1_Q5.png}};
  \draw[-{Latex[length=3.2mm,width=2.4mm]},line width=1.8pt,DeepNavy]
    ($(gapplot.south west)!0.34!(gapplot.south east)+(0,3.95cm)$) --
    ($(gapplot.south west)!0.34!(gapplot.south east)+(0,3.15cm)$);
\end{tikzpicture}
\source{Eurostat and national statistical institutes; authors' calculations. Units: no-policy year-on-year inflation, in percent; Jan. 2021--July 2026.}
\end{frame}
