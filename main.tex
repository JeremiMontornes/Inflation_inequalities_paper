...
2022 and is close to zero in 2021 and 2023. Over the same period, cumulative inflation for households aged 60 or over exceeds that
for households under 30 by 1.3 points, while the cumulative rural--city gap is 1.0 point.
...
0.1 point in 2021, 0.9 point in 2022, and 0.0 point in 2023; the cumulative age and rural--city gaps
over 2021--2023 are 1.3 and 1.0 points. The inflation burden is 3.1, 10.2, and 7.4 percent of net income for the
...
substantial dispersion within those groups \citep{kaplan2017inflation,argente2021cost}.
Complementing this expenditure-share approach, \citet{ampudia2024shopping} show how household
shopping behavior shapes the effect of monetary policy on inflation heterogeneity along the income distribution.
More recently, \citet{jaravel2024distributional} constructs long-run distributional 
consumer price indices for the United States, closely following the BLS methodology. For Europe,
...
sensitivity of the results to alternative rent mappings. For the euro area, the 2021--2023
income-quintile inflation gap in this robustness exercise is 0.9 point under our baseline mapping of HBS actual
...
segmented by age and residence area. Cumulative inflation is 15.2
%To ensure comparability with the 0.9-point cumulative Q1--Q5
%gap, we compute all three gaps from chained price-index changes between June 2021 and June 2023,
%using indices based in 2020 and the same annual HICP country weights. 
percent for households aged 60 or over and 13.9 percent for households under 30, yielding an age gap
of 1.3 points. It is 15.3 percent in rural areas and 14.3 percent in cities, yielding a rural--city
gap of 1.0 point. The monthly annual inflation rates in the figure show that both gradients vary
over time and temporarily narrow or reverse as energy inflation recedes.
...
age (panel a) and residence area (panel b), January 2020 to July 2026. The age and residence-area
cumulative gaps cover 2021--2023, use June endpoints within those years, and are calculated as
...
We apply the observed extra-EU import-price changes to the corresponding imported final goods and
intermediate inputs. The FIGARO production network then propagates the intermediate-input cost
impulses across EA20 countries and industries. We map the resulting producer-price responses to
consumption products and aggregate them with the same household expenditure weights used elsewhere
in the paper. The difference between the effects for Quintile 1 and Quintile 5 measures the effect
on inflation inequality.

For directly imported household goods, the price impulse is
$d_{di}^{k}=h_k F_{di}^{\mathrm{extra}}/F_{di}^{\mathrm{all}}$. For intermediate inputs, the full
EA20 price response is $\Delta\mathbf p^{k}=(\mathbf I-\mathbf A^{\mathsf T})^{-1}\mathbf b^{k}$.
Using the normalized NACE--COICOP bridge $\mathbf M$ and household weights $\mathbf w_g$, the total
effect is
\begin{equation}
\Delta\pi_g
=\mathbf w_g^{\mathsf T}\mathbf M\mathbf d^{k}
+\mathbf w_g^{\mathsf T}\mathbf M\Delta\mathbf p^{k}.
\label{eq:import-shock-household-effect}
\end{equation}
The first term is the direct effect and the second is the production-network effect. The effect on
inflation inequality is $\Delta\pi_{Q1-Q5}=\Delta\pi_{Q1}-\Delta\pi_{Q5}$.
...
Quintile 1--Quintile 5 gap. The observed and
counterfactual decompositions use the same product groups, weights, and aggregation rules.
...
private transport. Combining both instruments in a single energy-policy total therefore conceals
different distributional incidence: measures with similar effects on average inflation need not
...
0.9 point in 2022 and is close to zero in 2021 and 2023. Over 2021--2023, cumulative inflation is
1.3 points higher for households aged 60 or over than for those under 30 and 1.0 point higher for
rural than for urban households.
...
household energy. The measures lower the retail energy prices to which lower-income households
