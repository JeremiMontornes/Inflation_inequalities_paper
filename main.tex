...
We examine how recent energy-price shocks and the fiscal response shaped household inflation
...
points. The household-level inflation burden was 3.1, 10.2, and 7.4 percent of net income for the
...
shock. The rural--urban gradient is large in 2021--22 but disappears in 2023, whereas the age
gradient remains visible in 2022--23; the association with income changes sign over time. Finally, household price
policies lowered average cumulative inflation by 0.7 point and reduced the aggregate income-quintile
gap by 1.2 points. The mechanism is concentrated in household energy: the policies reduced the
contribution of water, electricity, gas, and other fuels to the income gap by 1.4 points, while
their effect through transport increased the gap by 0.2 point. A complementary input--output
...
wealth, income, and consumption \citep{pallotti2024bears}. Inflation erodes the real value of nominal
assets but benefits indebted households with fixed-rate loans
\citep{doepke2006inflation,cardoso2022heterogeneous}. It also reduces the purchasing power of income
when earnings and transfers are not fully indexed to prices; tax-benefit microsimulation studies
measure this channel by combining household microdata with fiscal measures and indexation rules
\citep{amores2025inflation}. This paper focuses on the consumption channel: households buy different
baskets and therefore face different inflation rates when relative prices change
\citep{jaravel2021inflation,cravino2020household,argente2021cost}. Existing studies measure this
channel using either category-level indices for household groups \citep{hobijn2005inflation} or more
granular household expenditure and price data \citep{kaplan2017inflation}.

Consumption-based inflation inequality requires both relative-price changes and heterogeneous
...
Adapting the distributional Consumer Price Index approach developed for the United States by
\cite{jaravel2024distributional} to the euro area requires one important modification. In the United
States, the CPI weights are derived directly
...
comparable with Eurostat inflation. The calibration is supported by internal aggregation checks and
by an external validation for France: the recalculated inflation series by standard-of-living
decile closely track the corresponding official INSEE series (Appendix~\ref{sec:app-validation-tests}).
This calibration yields harmonized public indicators by income, age, and residence area that can be
updated monthly. Relative to \cite{pallotti2024bears}, who assess the wealth, income, and consumption
channels of the 2021--2023 shock, we isolate variation arising from consumption baskets. The paper is
also related to policy work by \cite{bobasu2023impact} and \cite{claeys2022inflation}, who analyze
inflation inequality at the euro-area level.
...
Our empirical approach measures HBS budget shares at both the group and household levels while
holding item-level price changes common across households. It therefore captures variation in
expenditure shares rather than household-specific prices within narrowly defined products. The
group-level indices document differences between income, age, and residence categories; the
household-level indices reveal dispersion within those broad categories.
...
Together, these indicators distinguish two questions: who
is most exposed to a relative-price shock, and how recent unconventional fiscal policies that lower average inflation also
reduce inflation inequality.
...
s_{g,y}
=
\frac{E^{\mathrm{HBS}}_{g,y'}}{\sum_h E^{\mathrm{HBS}}_{h,y'}}.
\end{equation}
The RAS calibration imposes:
\begin{equation}
\sum_{j \in \mathcal{J}_y} M_{j,g,y}
=
s_{g,y}\sum_{j \in \mathcal{J}_y}\omega^{\mathrm{HICP}}_{j,y}
\quad \text{for all } g,
\qquad
\sum_g M_{j,g,y}
=
...
The product margin, $\sum_g M_{j,g,y}=\omega^{\mathrm{HICP}}_{j,y}$, ensures that the expenditure-weighted
...
Table~\ref{tab:ea20-income-gap-product-contributions} shows why the aggregate income gap remained
small between June 2021 and June 2023. Energy used in housing raised the Quintile 1--Quintile 5 gap
by 1.5 points and food added 0.7 point, but transport reduced it by 1.2 points.
...
To separate household purchases of energy from price propagation through production networks, we
use an input--output decompositiont. Let $\mathbf{A}$ denote the matrix of intermediate-input
...
let $\mathbf{w}_g$ denote the COICOP expenditure weights of household group $g$. The corresponding
...
The simulation imposes a uniform 27 percent production-price shock in every euro-area country on
...
\input{tables/tab_shock_B_C19_D35_27pct_EA20_product_2021_2023}
\endgroup
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
This decomposition clarifies the incidence of the fiscal measures studied below. The energy shock
widens the income gap mainly because lower-income households devote more of their budgets to
...
no-policy tariffs, statutory tax rates, or statutory unit rebates require no calibration: their
affected component, policy window, and monthly price wedge come directly from the underlying
sources, and we apply the observed or statutory ratio with unit intensity. Calibration enters only
when the inventory identifies an intervention but lacks the monthly tariff path, unit price, or
coverage weights needed for a direct reconstruction. In these cases, we choose one
...
no-policy reference tariff when available. Otherwise, we calibrate an aggregate-equivalent price
wedge. Second, for VAT and excise changes, we back out the reduced tax-inclusive price and reapply
the normal rate; a coverage and pass-through parameter is needed only when full statutory coverage
cannot be established. Third, for unit subsidies and fuel rebates, we add the statutory absolute
price wedge back to the observed monthly price path; when no retail unit-price series is available,
the wedge is calibrated in HICP component-index units. Evidence from
...
For transparency, the generic calibration can be written as follows. Let $\bar R_k$ denote the
audited aggregate-equivalent no-policy-to-observed price ratio for measure $k$, let $\mathcal{T}_k$
denote its active months, and let $w_{k,t}$ denote the normalized monthly wedge implied by the
available price information. We choose $\theta_k$ as the solution to
\begin{equation}
\exp\!\left\{\frac{1}{|\mathcal{T}_k|}
\sum_{t\in\mathcal{T}_k}\log\!\left(1+\theta_k w_{k,t}\right)\right\}
=\bar R_k .
\end{equation}
For an external reference-price path, $w_{k,t}$ is the positive reference-to-observed price wedge.
For a subsidy without a monthly unit-price series, $w_{k,t}=\bar P_j/P_{j,t}$, so a constant support
amount in index units produces a larger proportional wedge when the observed component index is
lower. For a tax measure with statutory gross-price factor
$S_k=(1+\tau^0_k)/(1+\tau^1_k)$, the equivalent coverage and pass-through parameter has the closed
form $\theta_k=\log(\bar R_k)/\log(S_k)$ and the implied ratio is $S_k^{\theta_k}$. If neither a
monthly reference path nor a statutory profile exists, the calibrated wedge is constant and equals
$\bar R_k$. Thus, calibration preserves the target average effect while allowing the available
monthly price profile to determine its timing.
...
wedges when a statutory reconstruction is incomplete. The country inventory identifies the layer used
...
fiscal-cost column sums targeted and untargeted household price-policy budgets. These budget figures
provide context for the scale of the interventions; they are not inputs into the price-index
counterfactual and do not measure the monetary benefit received by each household group. We
therefore do not interpret the ratio of a country's reported budget to its estimated change in the
inflation gap as a measure of cost-effectiveness. Such a comparison would require a microsimulation
that allocates fiscal costs and benefits across households and expresses both in euros.
Table~\ref{tab:country-heterogeneity-cumulative-gap} reports the observed inflation outcomes.
...
Without the simulated price-based fiscal interventions, inflation for the first income quintile
would have risen further above inflation for the fifth quintile. Figure~\ref{fig:ea-counterfactual-income-inflation}
shows the corresponding monthly no-policy paths.
...
Using unrounded indices, the resulting policy effect is $-1.2$ points. For comparison, the U.S.
distributional CPIs of \citet{jaravel2024distributional} imply cumulative inflation of 13.0 percent
for the first income quintile and 13.5 percent for the fifth between May 2020 and May 2022, a
Q1--Q5 gap of $-0.5$ point. The U.S. profile is inverse U-shaped---inflation reaches 14.7 percent
for the middle class---rather than monotonically declining with income. This comparison underscores
that the sign and size of inflation inequality depend on both the composition of the price shock and
the policies affecting the most exposed categories.

The product decomposition identifies the mechanism behind the $-1.2$-point effect. Household price
policies reduce the contribution of water, electricity, gas, and other fuels to the Q1--Q5 gap by
1.4 points. Their effect through transport instead increases the gap by 0.2 point, because fuel-price
measures also benefit the transport-intensive baskets of higher-income households. Effects through
the remaining product groups are close to zero. This pattern mirrors the input--output simulation:
both exercises identify household energy as the dominant distributional channel, while transport
partly offsets it. The fiscal measures thus dampen the same direct exposure that generates most of
the simulated energy-shock gap; they do less to undo the smaller component transmitted indirectly
...
The cross-country figures should consequently not be read as a ranking of policy efficiency. For
example, France combines a reported budget of EUR 80.5 billion with a 2.6-point reduction in the
Q1--Q5 inflation gap, whereas Slovakia combines EUR 2.3 billion with a 3.0-point reduction. These
figures refer to economies of different sizes, policy coverage, energy systems, and initial price
exposures. Scaling budgets by population or GDP would improve comparability of fiscal effort, but
would still not identify cost-effectiveness: the inflation-gap effect is measured in percentage
...
no household-level basket can be matched to the annual HICP series at the selected COICOP level.\\
...
\vspace{4pt}
\captionsetup{font=footnotesize, format=hang, singlelinecheck=false, justification=justified}
\caption*{Notes: The burden is computed for each household as its annual basket-specific inflation rate multiplied by its consumption-to-net-income ratio. Ratios are winsorized at the survey-weighted national 99th percentile; income quintiles are defined within countries. Estimates use the 2020 HBS microdata and annual inflation for 2021--2023. Italy is excluded because the harmonized micro-income input is unavailable. Portugal is absent because no household-level basket can be matched to the annual HICP series at the selected COICOP level. The resulting sample covers 18 euro-area countries.\\
Sources: Eurostat HICP and HBS microdata; authors' calculations.}
\end{figure}
...
residence areas than across income quintiles in the euro-area aggregate. Between June 2021 and June
2023, the cumulative Quintile 1--Quintile 5 gap was 0.9 point, compared with 1.3 points between
households aged 60 or over and those under 30, and 1.0 point between rural and urban households.
These differentials were concentrated around the energy-price surge and narrowed as energy
inflation receded.
...
lowered average cumulative inflation by 0.7 point and reduced the aggregate Quintile 1--Quintile 5
gap by 1.2 points in the no-policy comparison.

The two decompositions reveal why these policies reduced inequality. In the policy counterfactual,
lower household-energy prices reduce the Q1--Q5 gap by 1.4 points, while the transport channel
increases it by 0.2 point. In the input--output simulation, direct household exposure accounts for
0.7 point of the 1.0-point gap generated by a common energy-sector shock, and production-network
propagation accounts for 0.3 point. Nonconventional fiscal measures were therefore distributionally
effective mainly because they targeted the retail energy prices to which lower-income households
were directly exposed. Their weaker leverage over indirect cost propagation also delineates the
limits of price-based intervention.
...
Price-based fiscal measures can therefore provide temporary distributional stabilization during
large relative-price shocks, while monetary policy remains responsible for aggregate price
stability. Income alone does not identify vulnerable households: residence and age also capture
differences in heating technology, mobility constraints, and the ability to substitute between
products.

More granular transaction and expenditure data would allow future work to measure substitution
within product categories and combine consumption exposure with income indexation and household
balance sheets. A household tax-benefit and energy-consumption microsimulation could also allocate
